...
<div class="container mx-auto px-4 py-8">
  <div class="text-center mb-12">
    ...
    <p class="text-xl text-black opacity-90">Book celebration packages for traditional Indian festivals</p>
  </div>

  <?php
  // Fetch only active events in category "Festivals"
  $stmt = $con->prepare("SELECT event_id, name, description, price, event_date, event_image FROM events WHERE status='active' AND category='Festivals' ORDER BY event_date ASC");
  $stmt->execute();
  $res = $stmt->get_result();
  ?>
